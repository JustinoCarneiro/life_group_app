...
@RestController @RequestMapping("/api/lifegroups")
public class LifeGroupControlador {
    @Autowired private LifeGroupServico lifeGroupServico;
    ...
    public List<LifeGroupDTO> listarLifeGroupsPorSetor(@PathVariable UUID idSetor) {
        return lifeGroupServico.buscarLifeGroupsPorSetor(idSetor).stream().map(this::converterParaDTO).collect(Collectors.toList());
    }
    @PostMapping
