...
public class AreaControlador {
    @Autowired private AreaServico areaServico;
    private AreaDTO converterParaDTO(Area area) { return new AreaDTO(area.getId(), area.getNome()); }
    @GetMapping
    public List<AreaDTO> listarAreas() {
        return areaServico.buscarTodasAreas().stream().map(this::converterParaDTO).collect(Collectors.toList());
    }
    @PostMapping
