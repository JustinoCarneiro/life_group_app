...
public class SetorServico {
    ...
    public Setor criarSetor(SetorCriarDTO dto) {
        // Busca a entidade Area completa para criar a associação
        Area area = areaRepositorio.findById(dto.areaId())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Área não encontrada com id: " + dto.areaId()));
    ...
    public List<Setor> buscarSetoresPorArea(UUID idArea) {
        return setorRepositorio.buscarPorIdArea(idArea);
    }
